...
using BusinessDocumentStatus = BusinessLogic.DTOs.DocumentStatus;
using BusinessUserRole = BusinessLogic.DTOs.UserRole;
    ...
    IMapper mapper) : IDocumentService
{
    ...
    public async Task ProcessAsync(Guid documentId, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var pendingChunks = pages
                .SelectMany(page => ChunkByStrategy(page.Text, document.ChunkingStrategy, rag.ChunkWords, rag.OverlapWords)
                    .Select(text => (page.Page, Text: text)))
                .ToList();
            for (var index = 0; index < pendingChunks.Count; index++)
            {
                var pending = pendingChunks[index];
                var vector = await embeddings.EmbedAsync(pending.Text, EmbeddingTask.Document, document.EmbeddingModel, ct);
    ...
    private static IEnumerable<string> ChunkByStrategy(string text, string strategy, int size, int overlap)
    {
        return strategy switch
        {
            "Sentence" => ChunkBySentence(text, size),
            "Paragraph" => ChunkByParagraph(text, size),
            _ => Chunk(text, size, overlap)   // "WordCount" (mặc định)
        };
    }
    ...
    private static IEnumerable<string> ChunkBySentence(string text, int maxWords)
    {
        // Tách câu theo dấu kết thúc câu; gộp các câu nhỏ lại cho đến khi đủ maxWords
        var sentences = Regex.Split(text.Trim(), @"(?<=[.!?])\s+").Where(s => s.Length > 0);
        var buffer = new System.Text.StringBuilder();
        var wordCount = 0;
        foreach (var sentence in sentences)
        {
            var wc = sentence.Split(' ', StringSplitOptions.RemoveEmptyEntries).Length;
            if (wordCount + wc > maxWords && buffer.Length > 0)
            {
    ...
    private static IEnumerable<string> ChunkByParagraph(string text, int maxWords)
    {
        // Tách theo đoạn (dòng trống); nếu đoạn quá dài thì fallback sang WordCount
        var paragraphs = Regex.Split(text.Trim(), @"(\r?\n){2,}").Where(p => p.Trim().Length > 0);
        foreach (var para in paragraphs)
        {
            var wc = para.Split(' ', StringSplitOptions.RemoveEmptyEntries).Length;
            if (wc <= maxWords)
                yield return para.Trim();
            else
                foreach (var sub in Chunk(para, maxWords, 0))
                    yield return sub;
        }
    }
