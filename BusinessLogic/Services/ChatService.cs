...
using BusinessMessageRole = BusinessLogic.DTOs.MessageRole;
using BusinessUserRole = BusinessLogic.DTOs.UserRole;
using DataDocumentStatus = BusinessObjects.Entities.DocumentStatus;
using DataMessageRole = BusinessObjects.Entities.MessageRole;
    ...
    IMapper mapper) : IChatService
{
    ...
    public async Task<ChatAnswer> AskAsync(Guid userId, BusinessUserRole role, Guid courseId, Guid? conversationId, Guid? documentId, string question, CancellationToken ct = default)
    {
        ...
        try
        {
            var query = await embeddings.EmbedAsync(trimmedQuestion, EmbeddingTask.Query, null, ct);
            ...
            var historyContext = history.Select(x => new AnswerMessageContext((DataMessageRole)x.Role, x.Content)).ToList();
