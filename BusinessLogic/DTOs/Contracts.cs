...
public sealed record DocumentDto(Guid Id, Guid CourseId, string CourseName, string FileName, DocumentStatus Status, string? FailureReason, DateTime UploadedAtUtc, string ChunkingStrategy = "WordCount", string EmbeddingModel = "gemini-embedding-001");
...
public sealed record UploadRequest(Guid CourseId, Guid UploadedById, string FileName, string ContentType, Stream Content, bool Overwrite, string ChunkingStrategy = "WordCount", string EmbeddingModel = "gemini-embedding-001");
