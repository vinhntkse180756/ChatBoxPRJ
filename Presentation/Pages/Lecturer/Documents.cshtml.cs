...
public sealed class DocumentsModel(ICourseService courses, IDocumentService documents) : PageModel
{
    ...
    [BindProperty] public string ChunkingStrategy { get; set; } = "WordCount";
    [BindProperty] public string EmbeddingModel { get; set; } = "gemini-embedding-001";
    ...
    public async Task<IActionResult> OnPostUploadAsync()
    {
        ...
        var result = await documents.UploadAsync(new UploadRequest(UploadCourseId.Value, User.UserId(), UploadFile.FileName, UploadFile.ContentType, stream, Overwrite, ChunkingStrategy, EmbeddingModel), HttpContext.RequestAborted);
